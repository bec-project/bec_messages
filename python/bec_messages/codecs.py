...
from bec_messages import messages as messages_module
from bec_messages import numpy_encoder
from bec_lib.device import DeviceBase
from bec_messages.endpoints import EndpointInfo
from .messages import BECMessage, BECStatus
...
class BECCodec(ABC):
    ...
    @staticmethod
    @abstractmethod
    def decode(type_name: str, data: dict) -> Any:
        """Decode data into an object."""


class NumpyEncoder(BECCodec):
    obj_type = [np.ndarray, np.bool_, np.number, complex]
...
class BECTypeEncoder(BECCodec):
    ...
    def decode(type_name: str, data: dict) -> type:
        ...
        if data["module"] == "bec_messages.messages":
